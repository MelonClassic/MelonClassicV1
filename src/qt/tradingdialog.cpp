    ...
    ui(new Ui::tradingDialog)
{
    ...
    ui->MelonClassicAvailableLabel->setPalette(sample_palette);
    ...
    CreateOrderBookTables(*ui->BidsTable,QStringList() << "TOTAL(BTC)"<< "MelonClassic(SIZE)" << "BID(BTC)");
    CreateOrderBookTables(*ui->AsksTable,QStringList() << "ASK(BTC)"  << "MelonClassic(SIZE)" << "TOTAL(BTC)");
    /*OrderBook Table Init*/
    ...
    ui->MarketHistoryTable->setHorizontalHeaderLabels(QStringList()<<"DATE"<<"BUY/SELL"<<"BID/ASK"<<"TOTAL UNITS(MelonClassic)"<<"TOTAL COST(BTC");
...
void tradingDialog::UpdaterFunction(){
    //MelonClassicst get the main exchange info in order to populate qLabels in maindialog. then get data
...
QString tradingDialog::GetMarketSummary(){
     ...
     QString Response = sendRequest("https://bittrex.com/api/v1.1/public/GetMarketSummary?market=BTC-MelonClassic");
     return Response;
}

QString tradingDialog::GetOrderBook(){

      QString  Response = sendRequest("https://bittrex.com/api/v1.1/public/getorderbook?market=BTC-MelonClassic&type=both&depth=50");
      return Response;
}

QString tradingDialog::GetMarketHistory(){
      QString Response = sendRequest("https://bittrex.com/api/v1.1/public/getmarkethistory?market=BTC-MelonClassic&count=100");
      return Response;
}
...
QString tradingDialog::BuyMelonClassic(QString OrderType, double Quantity, double Rate){
    ...
    QString URL = "https://bittrex.com/api/v1.1/market/";
            ...
            URL += "?apikey=";
            URL += this->ApiKey;
            URL += "&nonce=12345434&market=BTC-MelonClassic&quantity=";
...
QString tradingDialog::SellMelonClassic(QString OrderType, double Quantity, double Rate){
...
QString tradingDialog::GetOpenOrders(){
    QString URL = "https://bittrex.com/api/v1.1/market/getopenorders?apikey=";
            URL += this->ApiKey;
            URL += "&nonce=12345434&market=MelonClassic-BTC";
...
QString tradingDialog::GetBalance(QString Currency){
    ...
    QString URL = "https://bittrex.com/api/v1.1/account/getbalance?apikey=";
            URL += this->ApiKey;
            URL += "&nonce=12345434&currency=MelonClassic";
            URL += Currency;
...
QString tradingDialog::GetAccountHistory(){
    ...
    QString URL = "https://bittrex.com/api/v1.1/account/getorderhistory?apikey=";
            URL += this->ApiKey;
            URL += "&nonce=12345434&market=MelonClassic-BTC&count=10";
...
int tradingDialog::SetExchangeInfoTextLabels(){
    ...
    ui->volumet->setText("<b>MelonClassic Volume:</b> <span style='font-weight:bold; font-size:14px; color:blue;'>" + str.number(obj["Volume"].toDouble(),'i',8) + "</span> MelonClassic");
...
void tradingDialog::ParseAndPopulateOrderBookTables(QString OrderBook){
    ...
    double MelonClassicSupply = 0;
    double MelonClassicDemand = 0;
    double BtcSupply  = 0;
    double BtcDemand  = 0;
    ...
    foreach (const QJsonValue & value, SellArray)
    {
        ...
        MelonClassicSupply = MelonClassicSupply + y;
    ...
    foreach (const QJsonValue & value, BuyArray)
    {
        ...
        MelonClassicDemand = MelonClassicDemand + y;
        ...
        ui->MelonClassicSupply->setText("<b>Supply:</b> <span style='font-weight:bold; font-size:14px; color:blue'>" + str.number(MelonClassicSupply,'i',8) + "</span><b> MelonClassic</b>");
        ...
        ui->MelonClassicDemand->setText("<b>Demand:</b> <span style='font-weight:bold; font-size:14px; color:blue'>" + str.number(MelonClassicDemand,'i',8) + "</span><b> MelonClassic</b>");
    ...
    switch (index){
                ...
                case 1: //sell tab active
                                   ...
                                   if(Response.size() > 0 && Response != "Error"){
                                       ...
                                       QString balance = GetBalance("MelonClassic");
                                       QString str;
                                       QJsonObject ResultObject =  GetResultObjectFromJSONObject(balance);

                                       ui->MelonClassicAvailableLabel->setText(str.number(ResultObject["Available"].toDouble(),'i',8));
                                     }
                ...
                case 6://show balance tab
                       Response = GetBalance("BTC");
                         ...
                         Response = GetBalance("MelonClassic");

                       if(Response.size() > 0 && Response != "Error"){
                         DisplayBalance(*ui->MelonClassicBalanceLabel,*ui->MelonClassicAvailableLabel,*ui->MelonClassicPendingLabel, QString::fromUtf8("MelonClassic"),Response);
                        }
                break;
...
void tradingDialog::CalculateSellCostLabel(){
    ...
    double Quantity = ui->UnitsInputMelonClassic->text().toDouble();
    double cost = ((price * Quantity) - ((price * Quantity / 100) * 0.25));
...
{
    //calculate amount of BTC that can be gained from selling MelonClassic available balance
    QString responseA = GetBalance("MelonClassic");
    QString str;
    QJsonObject ResultObject =  GetResultObjectFromJSONObject(responseA);

    double AvailableMelonClassic = ResultObject["Available"].toDouble();

    ui->UnitsInputMelonClassic->setText(str.number(AvailableMelonClassic,'i',8));
}
...
void tradingDialog::on_BuyMelonClassic_clicked()
{
    double Rate;
    double Quantity;
    ...
    reply = QMessageBox::question(this,"Buy Order",Msg,QMessageBox::Yes|QMessageBox::No);
            ...
            if (reply == QMessageBox::Yes) {
                ...
                QString Response =  BuyMelonClassic(Order,Quantity,Rate);
...
void tradingDialog::on_SellMelonClassicBTN_clicked()
{
    double Rate;
    double Quantity;

    Rate     = ui->SellBidPriceEdit->text().toDouble();
    Quantity = ui->UnitsInputMelonClassic->text().toDouble();
    ...
    QString Msg = "Are you sure you want to Sell ";
            Msg += ui->UnitsInputMelonClassic->text();
            Msg += " MelonClassic @ ";
    ...
    reply = QMessageBox::question(this,"Sell Order",Msg,QMessageBox::Yes|QMessageBox::No);
            ...
            QString Response =  SellMelonClassic(Order,Quantity,Rate);
            QJsonDocument jsonResponse = QJsonDocument::fromJson(Response.toUtf8());          //get json from str.
            QJsonObject  ResponseObject = jsonResponse.object();                              //get json obj
...
void tradingDialog::on_UnitsInputMelonClassic_textChanged(const QString &arg1)
{
     CalculateSellCostLabel(); //update cost
}
